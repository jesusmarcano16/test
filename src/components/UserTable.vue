<script setup>
import { computed } from 'vue';
import { useStore } from '../store/store';

const store = useStore();

const position = computed(() => {
  let contador = 1;
  let result = {};
  const data = [...store.sortedData];
  result[data[0].name] = contador;
  for (let i = 1; i < data.length; i++) {
    if (data[i].puntuacion !== data[i - 1].puntuacion) {
      contador++;
    }
    result[data[i].name] = contador;
  }
  return result;
});

const puntosFaltantes = (puntos) => {
  return 20 - puntos < 0 ? 0 : 20 - puntos;
};
</script>

<template>
  <section class="text-gray-600" v-if="store.sellersData.length > 0">
    <div class="container mx-auto px-5 py-4">
      <div class="mb-10 mt-10 flex w-full flex-col items-center justify-center text-center">
        <h1 class="mb-2 text-3xl font-medium text-gray-900 sm:text-4xl">Tabla de posiciones</h1>
        <svg class="max-w-full" width="447" height="30" viewBox="0 0 447 30" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path
            fill-rule="evenodd"
            clip-rule="evenodd"
            d="M189.622 3.576C168.647 4.50823 147.918 5.6545 127.703 6.90766C99.0876 8.68043 70.4829 10.5143 42.0507 13.418C33.1009 14.3349 24.0123 14.9157 15.1184 16.0618C9.53536 16.7801 2.13241 17.8039 1.16087 18.0178C0.652709 18.1401 0.431017 18.3237 0.359383 18.3848C-0.144298 18.8127 -0.0523948 19.2252 0.231905 19.5461C0.346072 19.6836 0.634768 20.02 1.43618 20.0658C55.0323 23.1987 109.837 17.0858 163.498 16.4439C256.556 15.3436 352.434 19.7143 444.753 29.2506C445.559 29.327 446.343 28.9448 446.455 28.3794C446.589 27.8292 446.007 27.2943 445.201 27.2179C352.725 17.6664 256.69 13.2803 163.453 14.3959C113.444 14.9919 62.4464 20.3562 12.3672 18.5223C13.5178 18.3695 14.6461 18.2166 15.6758 18.079C24.5339 16.9329 33.5845 16.3674 42.4985 15.4505C70.8702 12.5468 99.4167 10.7129 127.994 8.95546C163.453 6.75478 200.457 4.85975 237.752 3.97336C251.094 4.1109 264.391 4.24851 277.688 4.41661C306.454 4.78339 335.354 5.85317 364.053 7.28972C372.694 7.73291 381.334 8.19135 389.975 8.58869C392.841 8.72624 400.228 9.12351 401.258 9.09295C402.534 9.06238 402.78 8.32889 402.802 8.20663C402.869 7.93155 402.825 7.54946 402.198 7.22853C402.131 7.18269 401.728 7.02976 400.832 6.93807C348.651 1.52808 292.933 0.626557 237.797 1.92557C179.638 1.36012 121.256 1.11548 63.2457 0.855682C62.4151 0.855682 61.7368 1.31422 61.7301 1.87967C61.7257 2.44512 62.3949 2.90368 63.2254 2.91896C105.23 3.10235 147.448 3.28564 189.622 3.576Z"
            fill="black"
          />
        </svg>
      </div>
      <div class="mx-auto w-full overflow-auto lg:w-2/3">
        <table class="whitespace-nowrap	 w-full table-auto text-left">
          <thead>
            <tr class="bg-gray-100 text-gray-900">
              <th class="rounded-tl rounded-bl px-4 py-3 text-sm font-medium tracking-wider">Nombre</th>
              <th class="px-4 py-3 text-center text-sm font-medium tracking-wider">Puntuación</th>
              <th class="px-4 py-3 text-center text-sm font-medium tracking-wider">Posición</th>
              <th class="px-4 py-3 text-center text-sm font-medium tracking-wider">Puntos faltantes para ganar</th>
              <th class="hidden px-4 py-3 text-sm font-medium tracking-wider sm:table-cell">Observaciones</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="user in store.sortedData" class="border-t-2 border-gray-200" :key="user.name">
              <td class="px-4 py-3 capitalize">{{ user.name }}</td>
              <td class="px-4 py-3 text-center">{{ user.puntuacion }}</td>
              <td class="px-4 py-3 text-center">{{ position[user.name] }}</td>
              <td class="px-4 py-3 text-center">{{ puntosFaltantes(user.puntuacion) }}</td>
              <td class="hidden px-4 py-3 text-gray-900 sm:block">{{ user.observations }}</td>
            </tr>
          </tbody>
        </table>
      </div>
    </div>
  </section>
</template>
