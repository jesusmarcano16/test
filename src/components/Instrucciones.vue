...
const ganador = computed(() => {
  return store.userData.find((x) => x.puntuacion > 20);
});
</script>

<template>
  <section v-if="store.ganador === false" class="text-gray-600">
    <div class="container mx-auto flex flex-col items-center justify-center px-5 pt-24">
      <div class="flex w-full flex-col items-center text-center md:w-2/3">
        <h1 class="mb-4 text-3xl font-medium text-gray-900 sm:text-4xl">Vendedores ¡a correr!</h1>
        ...
        <p class="leading-relaxed">
          Los vendedores de la empresa “Imágenes del mundo” deben traer de la web imágenes relacionadas a una palabra que el usuario ingresará en la
          aplicación, la imagen que más le guste al usuario ganará 3 puntos, el vendedor que complete 20 puntos ganará la carrera, el codiciado premio
          es la posibilidad de crear una factura en Alegra con los puntos acumulados por todos los vendedores durante la carrera.
        </p>
      </div>
    </div>
  </section>
  <div v-else-if="store.factura === false" class="container mx-auto flex flex-col items-center justify-center px-5 pt-24">
    <p>
      carrea completada el ganador es {{ ganador.name }} porfavor
      <router-link to="/facturar" class="text-indigo-500"> genera la factura </router-link>
    </p>
  </div>
</template>
